...
module MainCylinder(gramms)
{
    rotate([ 90, 0, 0 ]) union()
    {
        ...
        difference()
        {
            translate([ 0, 0, -extensionForHoldingClamps / 2 ]) union()
            {
                ...
                translate([ 0, 0, moveCutoff ])
                    cylinder(h = extensionForHoldingClamps, r2 = portionerRadius + wallThickness / 2,
                             r1 = portionerRadius, center = true);
            }

            translate([ 0, 0, moveCutoff ]) CutOffBackside();
        }

        // Front
        capThickness = wallThickness * 3;
        translate([ 0, 0, dispenserChamberHeight / 2 + capThickness / 2 ]) PortionerCap(gramms, width = capThickness);
    }
}
...
module CutOffBackside()
{
    union()
    {
        ...
        difference()
        {
            ...
            cube(
                [
                    dispenserChamberOuterDiameter + wallThickness + 0.1, spaceBetweenSlits,
                    cutoffHeightForHoldingClamps
                ],
                center = true);
        }

        // slits for making holding clamps flexible
        translate([ 0, spaceFromSlitToMiddle, 0 ]) cube(
            [ dispenserChamberOuterDiameter + wallThickness, slitHeight, cutoffHeightForHoldingClamps ], center = true);

        translate([ 0, spaceFromSlitToMiddle * -1, 0 ]) cube(
            [ dispenserChamberOuterDiameter + wallThickness, slitHeight, cutoffHeightForHoldingClamps ], center = true);
    }
}
...
module PortionerCap(gramms, width)
{
    ...
    union()
    {
        ...
        {
            // Cap
            color("#00ff00") cylinder(h = width, r = capRadius, center = true, $fn = 12);

            // Text
            translate([ 0, 0, inlayDepth ]) color("#ff0000") linear_extrude(inlayDepth)
                text(str(gramms, "g"), size = 10, halign = "center", valign = "center", font = dispenserFont);
        }

        fromMiddle = portionerRadius + 2 + wallThickness + tolerance * 2;
        stopperWidthOnCap = 2 * rotationStopperWidth;
        // Little stopper
        color("#00ff00") translate([ rotationStopperWidth / 2 + rotationStopperWidth, fromMiddle, -5 ])
            cube([ rotationStopperWidth * 2, 4, 4 ], center = true);
...
module SugarChamberSphericBottom()
{
    ...
    moveSugarChamber =
        radius /* to have the tip of the sphere on 0|0 */
        - radius / 2 -
        height /
            2 /* because the sphere-cylinder union is not center=true it is already halfway in the portioner cylinder */
        - tolerance;

    echo(str("radius: ", radius, " cylinder-height: ", height, " expected-volume: ", expectedVolume,
             " cylinder-volume: ", cylinderVolume, " half-sphere-volume: ", bottomHalfSphereVolume));

    translate([ 0, 0, moveSugarChamber ]) union()
    {
